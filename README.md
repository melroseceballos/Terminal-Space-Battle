...
1. Write the game in `app.js`
1. Test game using `node app.js`
1. Use `prompt-sync` to get user input and `console.log()` to send some sort of output to the user in the terminal.
...
Make it so the function reduces the target's hull by the firepower of the USS Schwarz.
...
Make many alien ships with objects. Make each object slightly different . . .
